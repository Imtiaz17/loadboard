    ...
    <div class="card">
        <div class="card-element">
            ...
            <div v-else style="padding:30px">
                <p style="font-size:22px;text-align:center;">Loading.....</p>
            </div>
        </div>
    </div>
</template>
...
export default {
    ...
    created() {
        ...
        {
            this.shipments=this.$store.getters.shipments
        }
        this.getShipments();
    },
    mounted() {
        EventBus.$on("ShipmentPosted", data => {
            this.shipments.unshift(data);
        });
    },
    methods: {
        getShipments() {
            this.$axios.get('/shipment')
                .then((res) => {
                     this.$store.dispatch('saveShipments', {
                                data: res.data.data
                            }, )
